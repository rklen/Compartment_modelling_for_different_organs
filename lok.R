...
fit_model<-function(param,k_number,input,j){
  ...
  if(length(param)==k_number+2){
    alpha<-1/(1+exp(-param[k_number+1]))
    Va<-1/(1+exp(-param[k_number+2]))
  }else{
      alpha<-1
      Va<-0
  }
...
modelNames<-c('1TCM','i2TCM','r2TCM',
              ...
              '1TCM+BVC','i2TCM+BVC','r2TCM+BVC',
              '1TCM+TDC+BVC','i2TCM+TDC+BVC','r2TCM+TDC+BVC')
...
#Models with blood volume correction only
j<-0
for(organIndex in 2:21){
  for(k_number in 2:4){
    df1<-matrix(data=NA,nrow=length(studyNumbers),ncol=9)
...
string<-''
for(organIndex in 2:21){
  aics<-matrix(data=NA,nrow=length(studyNumbers),ncol=12)
  for(l in 1:12){
    df1<-read.csv(paste(organIndex,'_',l,'.csv',sep=''))
    aics[,l]<-280*log(df1[,8])+2*c(2,3,4,3,4,5,4,5,6,5,6,7)[l]
  }
  ...
  if(n==10){
    string<-c(string,'-&')
    string<-c(string,wilcoxSymbol(aics[,10],aics[,11]),'&')
    string<-c(string,wilcoxSymbol(aics[,10],aics[,12]),'&')
    string<-c(string,wilcoxSymbol(aics[,10],aics[,7]),'&')
    string<-c(string,wilcoxSymbol(aics[,10],aics[,4]),'newRow')
  }
  if(n==12){
    string<-c(string,wilcoxSymbol(aics[,12],aics[,10]),'&')
    string<-c(string,wilcoxSymbol(aics[,12],aics[,11]),'&')
    string<-c(string,'-&')
    string<-c(string,wilcoxSymbol(aics[,12],aics[,9]),'&')
    string<-c(string,wilcoxSymbol(aics[,12],aics[,6]),'newRow')
  }
}
cat(string)
...
for(organIndex in 2:21){
  ...
  for(l in 1:7){
    if(l<=4){
      string<-c(string,'&',signif(mean(abs(df1[,l]))*60,3),'plusMinus',signif(sd(abs(df1[,l]))*60,3))
    }
    if(l==5){
      string<-c(string,'&',signif(mean(df1[,l]),3),'plusMinus',signif(sd(df1[,l]),3))
    }
    if(l>=6){
      if(bestModels[organIndex-1]<7){
        string<-c(string,'&-')
      }else{
        string<-c(string,'&',signif(mean(1/(1+exp(-df1[,l])))*100,3),'plusMinus',signif(sd(1/(1+exp(-df1[,l])))*100,3))
      }
    }
  }
  string<-c(string,'newRow')
